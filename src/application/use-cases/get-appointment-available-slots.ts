...
export interface GetAppointmentAvailableSlotsInput {
  ...
  date: Date
  serviceId: string
  // Al reprogramar una cita existente, se excluye su propio intervalo de las
  // horas ocupadas: si no, su propio horario actual nunca aparece disponible.
  excludeAppointmentId?: string
}

// Version "admin" de GetPublicSlotsUseCase: misma logica de huecos libres,
// pero permite excluir la cita que se esta reprogramando de los intervalos
// ocupados, para que reprogramar no obligue a buscar a ciegas un horario.
export class GetAppointmentAvailableSlotsUseCase {
  ...
  async execute(input: GetAppointmentAvailableSlotsInput): Promise<PublicSlotDto[]> {
    ...
    const busyIntervals = [
      ...appointments
        .filter(
          (a) =>
            (activeAppointmentStatuses as string[]).includes(a.status) &&
